...
CREATE TABLE purchases
 ...
 purchase_date TIMESTAMP DEFAULT NOW(),
     ...
     FOREIGN KEY (purchased_for) REFERENCES creator(creator_id) ON DELETE
     SET NULL,
         INDEX (fan_id), INDEX (wish_id), INDEX (purchased_for));

-- Create the creator's gift receiving history table

CREATE TABLE creator_gift_receiving_history ( receiving_id UUID DEFAULT uuid_generate_v4 () PRIMARY KEY,
                                                                                                    received_creator_id UUID REFERENCES creator(creator_id), received_from UUID REFERENCES fan(fan_id), received_number_of_wishes INT DEFAULT 0,
                                                                                                                                                                                                                                              received_wish_id UUID REFERENCES wish(wish_id), received_date TIMESTAMP DEFAULT NOW(), CONSTRAINT fk_received_creator_id
FOREIGN KEY (received_creator_id) REFERENCES creator(creator_id) ON
DELETE CASCADE,
       CONSTRAINT fk_received_from
FOREIGN KEY (received_from) REFERENCES fan(fan_id) ON
DELETE
SET NULL,
    CONSTRAINT fk_received_wish_id
FOREIGN KEY (received_wish_id) REFERENCES wish(wish_id) ON
DELETE
SET NULL,
